...
write(logObj,'debug','Initializing Simple Systems Management client');
...
%% check if SSM is available in the given region
availableRegions = RegionUtils.getRegionsForService('ssm');
if  ~contains(char(availableRegions), char(awsRegion.getName))
    write(logObj,'error',['Region: ',awsRegion.getName,' does not support SSM']);
end

%% build a handle to the client
builderH = AWSSimpleSystemsManagementClientBuilder.standard().withCredentials(awsCredentials).withClientConfiguration(obj.clientConfiguration.Handle);
