...
if ~isdeployed()
    ...
    iDisplayBanner('MATLAB Interface for AWS SSM');
...
if ~exist(commonDir, 'dir')
    ...
    error('AWS:SSM',['Could not locate common utilities at: ',commonDir]);
end
...
if ~exist(commonJarPath,'file') && ~exist(jarPath,'file')
    error('AWS:SSM', 'Could not locate jar file at: %s or %s',jarPath, commonJarPath);
end
...
function iAddInterface(rootDir)
iDisplayBanner('Adding MATLAB interface for AWS SSM Paths');
